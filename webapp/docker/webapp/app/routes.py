...
def prime(lower=0, upper=10000):
    ...
    response = requests.get(f'http://api/prime/{lower}/{upper}')
    p = response.json()

    return render_template('prime.html', primes=p['numbers'])
...
def cat():
    ...
    response = requests.get('http://api/cats')
    cat = response.json()

    return render_template('cats.html', cat=cat['url'])
